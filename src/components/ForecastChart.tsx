...
interface ForecastChartProps {
  data: Array<{ date: Date; revenue: number; isPredicted?: boolean }>;
}
...
export const ForecastChart = ({ data }: ForecastChartProps) => {
  ...
  const chartData = data.map((item, index) => {
    ...
    return {
      ...
      predictedWithConnection:
        item.isPredicted ||
        (prevItem && !prevItem.isPredicted && data[index + 1]?.isPredicted)
          ? item.revenue
          : null,
    };
  });
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Comparing actuals with ML predicted baseline.
          </p>
        </div>
