    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm">Total revenue breakdown by channel source</p>
        </div>
        <div className="flex gap-2 text-xs">
          <span className="px-3 py-1.5 bg-slate-100 rounded-md font-medium text-slate-600">Absolute</span>
          <span className="px-3 py-1.5 bg-white border border-slate-200 rounded-md font-medium text-slate-400">Percentage</span>
        </div>
      </div>
      
      <div className="space-y-6">
        {data.map((item) => (
          <div key={item.channel} className="flex items-center justify-between group">
            ...
            <div className="flex-1 px-4">
               <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                 <div 
                   ...
                   style={{ width: `${(item.revenue / Math.max(...data.map((d: any) => d.revenue))) * 100}%` }}
                 />
               </div>
            </div>
